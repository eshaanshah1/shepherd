#!/usr/bin/env node
// `pnpm smoke:mirror` — build the app, then drive the built app once.
//
...
spawnSync('pkill', ['-f', 'shepherd-smoke=mirror'], { stdio: 'ignore' });
...
try {
  const result = spawnSync(
    electronBinary,
    [entry, '--shepherd-smoke=mirror', `--shepherd-user-data=${userData}`],
